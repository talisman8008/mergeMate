...
export default function FriendlinessScore({ score, breakdown, small = false }) {
  ...
  return (
    ...
    >
      ...
      {!small && (
        <span className="text-[13px] text-[var(--text-secondary)] mt-1">
          Friendliness
        </span>
      )}

      {showTooltip && (
        <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 z-50 bg-[var(--canvas)] border border-[var(--border)] rounded-lg px-3 py-2 shadow-sm min-w-[220px] text-left pointer-events-none">
          <p className="text-[11px] font-semibold text-[var(--text-secondary)] mb-2">Breakdown</p>

          {hasBreakdown ? (
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between items-center gap-4">
                <span className="text-[12px] text-[var(--text-secondary)]">Maintainer Response</span>
              ...
              <div className="flex justify-between items-center gap-4">
                <span className="text-[12px] text-[var(--text-secondary)]">Beginner PR Acceptance</span>
                <span className="text-[12px] font-medium text-[var(--text)]">
                  {breakdown.beginner_merge_rate != null ? `${Math.round(breakdown.beginner_merge_rate * 100)}%` : '—'}
                </span>
              </div>
              <div className="flex justify-between items-center gap-4">
                <span className="text-[12px] text-[var(--text-secondary)]">Issue Freshness</span>
                <span className="text-[12px] font-medium text-[var(--text)]">
